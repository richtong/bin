...
while getopts "hdvk:fx:i:g:s:e:n:m:t:" opt; do
	case "$opt" in
	h)
...
elif (($(id -u "$NEW_USER") != "$NEW_UID")); then
	log_verbose "changing $NEW_USER to use $NEW_UID"
	if (($(id -u) == $(id -u "$NEW_USER"))); then
		echo >&2 "$SCRIPTNAME: cannot change the current user $USER"

	else
		OLD_UID=$(id -u "$NEW_USER")
		if ! sudo usermod -u "$NEW_UID" "$NEW_USER"; then
			echo >&2 "$SCRIPTNAME: could not change $NEW_USER"
			return
		fi
...
if [ -e "$NEW_AUTHORIZED_KEYS" ]; then
	log_verbose "found $NEW_AUTHORIZED_KEYS"
	AUTHORIZED_KEYS="$(cat "$NEW_AUTHORIZED_KEYS")"
	if ! sudo -u "$NEW_USER" grep -q "$AUTHORIZED_KEYS" "$SSHDIR_KEYS"; then
