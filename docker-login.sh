...
if [[ -e $HOME/.docker/config.json && -n $(jq '.auths[].auth' $HOME/.docker/config.json) ]] && ! $FORCE
then
    log_verbose config.json already has a logon do not overwrite
        exit
    fi

    if ostype mac
    then
        if [[ ! -e /Applications/Docker.app ]] && ! docker-machine active
        then
            if ! docker-machine status "$DOCKER_MACHINE" | grep Running
            then
                if ! docker-machine ls | grep "$DOCKER_MACHINE"
                then
    ...
    then
        ...
        log_warning to allow dev-ssh to use docker then sudo service iam-key restart
