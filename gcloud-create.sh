...
OPTIND=1
export FLAGS="${FLAGS:-""}"
while getopts "hdvb:s:a:o:p:k:" opt; do
	case "$opt" in
	h)
		cat <<-EOF
...
if (($(gcloud projects list --filter="projectId:$PROJECT" | wc -l) < 2)); then
	...
	if ! gcloud projects create "$PROJECT" --organization "$ORG" --set-as-default; then
...
if (($(gcloud beta billing projects describe "$PROJECT" --format="value(billingAccountName)" | wc -l) < 1)); then
...
if (($(gcloud iam service-accounts list \
	--filter="email:$account_email" | wc -l) < 2)); then
...
if [[ ! $(readlink -f "$SECRET_DIR") =~ ^$(readlink -f "$HOME") ]]; then
