...
if ! $FORCE
then
    log_warning dryrun: Will remove repos in $MAIN
fi
...
elif ! git submodule deinit -f "$repo"
    then
...
elif ! git commit -m "Deleted submodule $repo"
    then
