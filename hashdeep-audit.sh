...
if ! comm -3 "$DST.hashdeep.sorted" "$SRC.hashdeep.sorted"
then
    log_verbose seeing what diff says ignore diff error
    if ! diff "$DST.hashdeep.sorted" "$SRC.hashdeep.sorted"
    then
