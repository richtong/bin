...
OPTIND=1
while getopts "hdvw:u:e:w:k:r:m:l:" opt
do
    case "$opt" in
        h)
...
if $VERBOSE
then
    >&2 echo  "$SCRIPTNAME: $USER repo $WS_DIR"
    >&2 echo  "      git $GIT_USER ($GIT_EMAIL)"
    >&2 echo  "      docker $DOCKER_USER ($DOCKER_EMAIL)"
fi
...
if ! use_openssh_keychain "$GIT_KEY" "$LOCAL_KEY"
then
    ...
    if "$SCRIPT_DIR/set-profile.sh" -s "$HOME"
    then
        echo $SCRIPTNAME: updated configuration, please reboot and restart this script
        exit 4
    fi

    if ! "$BIN_DIR/install-git-and-repos.sh" -u "$GIT_USER" -e "$GIT_EMAIL" -k "$GIT_KEY"
    then
        log_warning could not setup repos
        exit 6
    fi

    # If there are other agent specific installations they go in personal
    if [ -f "$WS_DIR/git/user/agent/bin/install.sh" ]
    then
