...
if [ ! -d "$SCRIPT_SSH_DIR" ]; then
	log_error 1 "No ssh directory $SCRIPT_SSH_DIR found"
fi
...
for agent in $AGENTS; do
	...
	if ! ssh -t "$agent@localhost" \
		...
		"bin/install-agent.sh $LOG_FLAGS"; then
