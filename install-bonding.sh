...
if ! $FORCE && is_package_installed network_manager
then
...
log_warning do not run this from ssh we will bring the network down now
    ...
    if ! git_install_or_update Network-Interfaces-Script JoeKuan
    then
    ...
    if ! config_mark /etc/network/interfaces
    then
