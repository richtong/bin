...
if ! disks_already_mounted $DISKS
then
    ...
    if ! sudo btrfs filesystem show --all-devices | grep -q "$disk"
    then
...
then
    ...
    sudo mkdir -p "/$POOL"
    config_add_once /etc/fstab "$disk /$POOL btrfs defaults,compress=lzo 0 1"
fi
...
if ! config_mark /etc/samba/smb.conf
then
...
for share in $SHARES
do
    if ! sudo btrfs subvolume "/$POOL/$share" > /dev/null
    then
