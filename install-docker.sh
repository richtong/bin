...
FORCE="${FORCE:-false}"
EDGE="${EDGE:-true}"
while getopts "hdvctr:m:o:fns:" opt
do
    case "$opt" in
        h)
            cat <<EOF
...
if ! $FORCE && command -v docker
then
    INSTALLED_DOCKER="$(version_extract "$(docker -v)")"
    if vergte  "$INSTALLED_DOCKER" "$version_needed"
    then
...
then
    ...
    if ! command -v docker || ! vergte "$(version_extract "$(docker -v)")" "$version_needed"
    then
...
elif in_linux ubuntu
then
    package_install curl


    if ! command -v docker  || ! docker -v | grep "$version_needed"
    then
...
if $DOCKER_CONTENT_TRUST &&  ! grep "$DOCKER_CONTENT_TRUST" ~/.bashrc
then
...
install_docker_module()
{
    ...
    if ! command -v "$tool"  ||  verlt "$(version_extract "$("$tool" version)")" "$version"
    then
...
if ! config_mark "$HOME/.dockerfilelintrc"
then
