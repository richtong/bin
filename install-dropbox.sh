...
if in_os mac; then
    ...
    if [[ ! -e $dest/.dropbox-dist ]]; then
        # shellcheck disable=SC2086
        sudo tar $VERBOSE_FLAG -xf "$dropbox_tar" -C "$dest"
    fi

    if ! command -v dropbox.py; then
...
EOF
    ...
    if dropbox.py status | grep -q "isn.t running"; then
