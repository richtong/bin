...
OPTIND=1
while getopts "hdvt:u:s:r:e:nfm" opt; do
	case "$opt" in
	h)
		cat <<-EOF
...
if command -v port >/dev/null; then
	if ! port | grep "OS platform mismatch"; then
...
if ! command -v port; then
	...
	if [[ $macports_extension =~ tar.bz2 ]]; then
		...
		if ! pushd "$macports_dir" >/dev/null; then
			log_error 2 "no $macports_dir"
		fi
		./configure
		make
		...
		if ! config_mark; then
			# we want to defer evaluation of this line to .profile runtime
...
if $MIGRATION && port installed | grep -v "No ports are installed"; then
