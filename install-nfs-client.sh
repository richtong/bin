...
log_verbose "make sure $USER has correct $CORRECT_UID and $CORRECT_GID"
if [[ $(grep "[0-9][0-9]* $USER" ../etc/users.txt | awk '{print $1}') != $(id -u) ]]; then
	log_warning could not find user
fi

if grep -q "Added by $SCRIPTNAME" /etc/fstab; then
