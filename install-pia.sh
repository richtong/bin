...
then
    ...
    then
        ...
        if [[ ! -e "/Volumes/Private Internet Access" ]]
        then
            log_verbose no PIA installer volume found
            DMG=$(ls -t "$HOME"/Downloads/pia-v*-installer-mac.dmg | head -1)
            if [[ -z $DMG ]]
            then
...
then
    ...
    if dpkg -s network-manager-openvpn | grep -q "ok installed"
    then
        exit 0
    fi
