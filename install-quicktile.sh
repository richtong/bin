...
if [[ ! -e $HOME/.config/quicktile.cfg ]]
then
...
then
    ...
    do
        for key in "/xfwm4/$mode/<Primary><Alt>KP_"{1..9}
        do
            if xfconf-query -c xfce4-keyboard-shortcuts -p "$key"
            then
...
EOF
    ...
    if [[ ! -e $HOME/.config/autostart/quicktile.desktop ]]
    then
