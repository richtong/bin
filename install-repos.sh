...
while getopts "hdvw:f" opt; do
	case "$opt" in
	h)
		cat <<-EOF
			$SCRIPTNAME: flags: -d debug, -h help
			    -w WS directory
			    -f force reset to origin/master (default: $FORCE)
		EOF
		exit 0
		;;
	d)
...
set +o pipefail
if ! (exec ssh -o StrictHostKeyChecking=no -T git@github.com) 2>&1 | grep -q "successfully authenticated"; then
