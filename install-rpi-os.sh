...
export FLAGS="${FLAGS:-""}"
while getopts "hdvs:wo:i:p:" opt; do
	case "$opt" in
	h)
		cat <<-EOF
...
if ! diskutil info "$SD_LOC" | grep "Card Reader"; then
...
touch "$SD_MOUNT/ssh"
if [[ ! -e $SD_MOUNT/wpa_supplicant.conf ]]; then
