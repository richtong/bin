...
if brew_install ruby; then
	...
	if ! config_mark; then
		...
		config_add <<-EOF
			command -v brew >/dev/null && echo "\$PATH" | grep -q "\$(brew --prefix ruby)/bin" || PATH="\$PATH:\$(brew --prefix ruby)/bin"
			command -v brew >/dev/null && echo "\$PATH" | grep -q "\$(brew --prefix)/lib/ruby/gems" || PATH="\$PATH:\$(brew --prefix)lib/ruby/gem/$version/bin"
		EOF
	fi
