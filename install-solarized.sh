...
then
    ...
    if ! defaults read com.apple.Terminal "Window Settings" | grep -q 'name = "Solarized Dark"'
    then
        git_install_or_update "$MAC_REPO" "$MAC_GIT"
        log_warning will open Terminal windows to when adding solarized layouts
        for terminal in "$WS_DIR/git/$MAC_REPO"/*.terminal
        do
            open "$terminal"
        done
    fi
...
then
    ...
    for color_terminalrc in "$WS_DIR/git/$XFCE_REPO/"*/terminalrc
    do
    ...
    mkdir -p "$dest"
    if [[ ! -e $dest/terminalrc && -e $dark/terminalrc ]]
    then
        mkdir -p "$dest"
...
if [[ $(desktop_environment) =~ (xfce|unity|gnome) ]]
then
