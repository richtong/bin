...
if ! grep -q "Added by $SCRIPTNAME" /etc/ssmtp/ssmtp.conf
then
...
if ! grep -q "root:$EMAIL" /etc/ssmtp/revaliases
then
...
if ! sendmail -V 2>&1 | grep -q sSMTP
then
    echo $SCRIPTNAME: ssmtp did not install and is not emulating sendmail
    exit 1
fi
