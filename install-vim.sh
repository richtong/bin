...
OPTIND=1
while getopts "hdvfal" opt; do
	case "$opt" in
	h)
		cat <<-EOF
			$SCRIPTNAME: Install vim and plugins deprecated use install-nvim.sh
			Use chez-moi to install the init.lua and other files for LazyVIM instead
				flags:
				-h help
	...
	f)
		...
		;;
	l)
		LAZYVIM_INSTALL="$($LAZYVIM_INSTALL && echo false || echo true)"
		;;
	a)
...
log_verbose "Doing bare vim installation"
