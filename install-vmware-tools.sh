...
if dpkg -s open-vm-tools  | grep -q "install ok"
then
    ...
    if ! $FORCE && mountpoint -q /mnt/hgfs
    then
        ...
        # copy and paste work properly
        # http://kb.vmware.com/selfservice/microsites/search.do?language=en_US&cmd=displayKC&externalId=1005436
        if dpkg -s open-vm-tools  2>/dev/null | grep -q "install ok"
        then
            sudo apt-get purge -y open-vm-tools
        fi

        if $FORCE && command -v vmware-uninstall-tools.pl
        then
        ...
        if command -v vmware-toolbox
        then
            log_exit vmware present and running

        fi

        if command -v vmware-config-tools.pl
        then
        ...
        read -p "Please click on Mac VMWare Fusion app, Choose Virtual Machine/Install VMware Tools, press enter when done"
