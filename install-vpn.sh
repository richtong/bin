...
for ext in crt csr key; do
	if [[ ! -r $KEY_DIR/$KEY_USER.$ext ]]; then
		log_warning "Invalid $KEY_DIR missing $KEY_USER.$ext"
		should_exit=true
	fi
done
if "$should_exit"; then
...
for file in ca.crt ta.key; do
	if [[ ! -r $KEY_DIR/$file ]]; then
