...
MOUNTDIR="${MOUNTDIR:-"/"}"
OPTIND=1
while getopts "hdvp:s:fm:" opt; do
	case "$opt" in
	h)
...
for share in $DATASETS; do
	...
	if ! zfs_list name "$dataset" >/dev/null; then
	...
	if [[ $(zfs_list mountpoint "$dataset") == "$mountpoint" ]]; then
