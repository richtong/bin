...
flags=""
options=""
while getopts "hdvc:lp:s:a:fbq:u:xm:" opt; do
	case "$opt" in
	h)
...
# We default to 1/4
if ! grep -q zfs_arc_max /etc/modprobe.d/zfs.conf; then
...
if ! disks_not_mounted $DISKS >/dev/null; then
	log_verbose $? drives are already mounted skipping adding drives to a new pool
elif sudo zpool status "$POOL" 2>&1 | grep -q "pool: $POOL"; then
	...
	if in_vmware_fusion && [[ ! $flags =~ -f ]]; then
		flags+="-f"
	fi
	...
	# Note cache adds
	if [[ -n $CACHE ]] && disk_is_ssd $CACHE; then
		options+=" cache $CACHE"
	fi
...
if ! config_mark "$HOME/.profile"; then
...
# https://pthree.org/2012/12/31/zfs-administration-part-xv-iscsi-nfs-and-samba/
if ! config_mark /etc/exports; then
...
if sudo service nfs-kernel-server start | grep -q Starting; then
...
if sudo zfs share "$POOL" 2>&1 | grep "already started"; then
...
if ! sudo service iam-key status | grep running; then
