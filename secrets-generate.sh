...
USERS="${USERS:-"$USER"}"
SITES="${SITES:-"github.com amazon.com"}"
...
URLS="${URLS:= \
              ...
              "https://console.aws.amazon.com/iam/home?region=us-west-2#/users/$USER?section=security_credentials" \
              }"
DATES="${DATES:-"$(date +%Y-%m)"}"
SECRETS_DIR="${SECRETS_DIR:-"$HOME/.ssh"}"
CONFIG="${CONFIG:-false}"
FORCE="${FORCE:-false}"
FLAGS="${FLAGS:-""}"
while getopts "hdvfms:u:y:t:l:" opt
do
    case "$opt" in
        h)
            cat <<-EOF
...
log_verbose sites are $SITES
site_array=($SITES)

log_verbose user array $USERS
user_array=($USERS)

log_verbose date stampe $DATES
date_array=($DATES)

log_verbose key types $TYPES
type_array=($TYPES)

log_verbose urls $URLS
url_array=($URLS)


for i in $(seq 0 $(( ${#site_array[@]}-1 )) )
do

    log_verbose creating key, use the user or other element is you are short
    user="${user_array[i]:-$user}"
    site="${site_array[i]:-$site}"
