...
log_verbose then setup-ecryptfs-private create ~/.Private mounting into ~/Private
    log_verbose then ~/Dropbox.Private copies into ~/Private so we do not
            ...
            then
                ...
                log_verbose could not find $ENCRYPTED_PATH expanding search to all $FILESHARING_DIRS

                if [[ -z ${ENCRYPTED_PATHS-} ]]
                then
                    ...
                    then
                        ...
                        # the same as ${DIR[0]}
                        pushd "$(dirname "$FILE_SHARING_DIR")" >/dev/null
                        ...
                        log_error 2 "mount and edit as needed then rerun $SCRIPT_NAME"
                        popd > /dev/null
                    else
            ...
            if [[ $ENCRYPTED_MOUNTPOINT == $PRIVATE_MOUNTPOINT ]]
            then
                echo $ENCRYPTED_PATH
