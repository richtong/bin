...
log_verbose environment is $(set -o posix ; set )

# the set -u has to come later because SSH_AUTH_SOCK might not be set
set -u

# Make sure we have a docker login
if ! grep -q '"auth":' "$HOME/.docker/config.json"
then
>&2 echo $SCRIPTNAME: Not logged into docker cannot build
exit 1
fi


# Make sure we are using the correct keychain as gnome doesn't handle id_25519 keys
if ! use_openssh_keychain "$GIT_KEY"
then
echo $SCRIPTNAME: added openssh keychain, reboot required then restart script
    exit 3
fi
...
if  ! $FORCED && [[ "$local_commit" == "$remote_commit" ]]
then
    echo $SCRIPTNAME: No need to build, local same as remote
    exit 0
fi

echo $SCRIPTNAME Rebuilding
echo local repo at $local_commit
echo remote master at $remote_commit

if pgrep -u "$USER" wscons >/dev/null
then
    echo $SCRIPTNAME: wscons already running, exiting
    exit 0
fi
...
if ! "$WS_DIR/git/src/bin/wscons" $BUILD_ARGS
then
    >&2 echo $SCRIPTNAME: build failed trying with less concurrency \(-j 2\)
    if ! "WS_DIR/git/src/bin/wscons" -j 2 $BUILD_ARGS
    then
        echo $SCRIPTNAME: wscons $BUILD_ARGS FAILED building origin $REMOTE
...
if [[ ! -e $WS_DIR/var/shared/local/hub-ssdp.config ]]
then
    echo '{ "name": "'$HOSTNAME'" }' > \
        "$WS_DIR/var/shared/local/hub-ssdp.config"
fi
...
then
    ...
    set +o pipefail
    local cameras=$(grep "$HOSTNAME" "$WS_DIR/git/src/infra/etc/camera.txt" | \
        head -n 2 | cut -d " " -f 1 )
    set -o pipefail
    echo finished camera search
    if (( $(echo $cameras | wc -w)  != 2 ))
    then
