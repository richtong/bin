...
OPTIONS="${OPTION:-"--keyfiles "" --encryption="(Serpent(Twofish(AES))" \
    --hash=sha=512 --pim=0 --random-source=/dev/urandom --filesystem=fat "}"
SIZE="${SIZE:-32}"
OPTIND=1
while getopts "hdvu:fs:o:z:m:" opt
do
    case "$opt" in
        h)
            cat <<-EOF
...
if ! veracrypt -tl "$secret_mountpoint_outer" 2>&1 >/dev/null
then
    ...
    log_warning ther volume already mounted at $secret_mountpoint_outer
fi
...
if ! veracrypt -tl "$secret_mountpoint" 2>&1 >/dev/null
then
